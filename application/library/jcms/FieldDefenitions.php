...
namespace jcms;
/**
 * This class holds the information of all the custom fields in the content table.
 * - $fields -> all the non system fields
 * - $alternateTypes -> Alternate behaviors the admin can set on a content field
 * - static getForm functions -> We need a function for each type_handler & alternateType
 ...
 *   2) add the dtb name to the FieldDefenitions::$fields array(DB_NAME => TYPE_HANDLER)
 *        if the type handler is string getFormString wil be called to render the form part of this field
 *   3) add a FieldDefenitions::getForm[dtb_field_name] that returns a Zend_Form element to edit the field
 *
 ...
 *   2) add a FieldDefenitions::getForm[Typehandler] that returns a Zend_Form element to edit the fields
...
class FieldDefenitions {
    ...
    // array of field_db_name's as key's and type_handler's as values for the custom fields
...
// 	);
	...
	// array of field_db_name's as key's and type_handler's as values for the system fields
	public static $systemfields = array('code'=>'code', 'title'=>'title', 'url'=>'url');

	public static $alternateTypes = array('tinymce'=>'text', 'file'=>'string', 'image'=>'string');
	...
	public static function getFormUrl($metadata, $mode, $value = null){
		...
		$element->setLabel($metadata['label']);
		$element->addDecorators(FormsHelper::$formDecoratorText);
		if($mode == 'add') $element->addValidator(new ValidatorUnique(new \Default_Model_Content(), 'url', true));
		return $element;
	}
	...
	public static function getFormCode($metadata, $mode, $value = null){
		...
		$element->setRequired(!$metadata['nullable']);
		$element->setLabel($metadata['label']);
		if($mode == 'add') $element->addValidator(new ValidatorUnique(new \Default_Model_Content(), 'code'));
		$element->addDecorators(FormsHelper::$formDecoratorText);
		return $element;
	}
	...
	public static function getFormFile($metadata, $mode, $value = null){
	  ...
	  $_fileElementDecorator = array(
	      ...
	      'Errors',
	      'Description',
	      array('Label', array('tag'=>'th', 'placement'=>'PREPENT')),
	  ...
	  // if we are in edit mode, we have a object_id and are able to show a preview
